...
class CourseOverview(TimeStampedModel):
    ...
    @classmethod
    def _create_or_update_from_course(cls, course):
        """
        ...
        # Update the course overview record if it already exists.
    ...
    def load_from_module_store(cls, course_id):
        ...
        with store.bulk_operations(course_id):
            ...
            if isinstance(course, CourseDescriptor):
                course_overview = cls._create_or_update_from_course(course)
                try:
                    with transaction.atomic():
                        ...
                        CourseOverviewImageSet.create_or_update_for_course(course_overview, course)
    ...
    def get_from_id(cls, course_id):
        ...
        if course_overview and not hasattr(course_overview, 'image_set'):
            CourseOverviewImageSet.create_or_update_for_course(course_overview)
...
class CourseOverviewImageSet(TimeStampedModel):
    ...
    @classmethod
    def create_or_update_for_course(cls, course_overview, course=None):
        """
        ...
        try:
            image_set = cls.objects.get(course_overview=course_overview)
        except cls.DoesNotExist:
